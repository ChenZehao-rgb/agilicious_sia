...
TEST(RuntimeConfig, MinimalGeoModelCannotEnterMpcOrDynamicsEstimator) {
	PilotParams mpc;
	mpc.directory_ = repositoryRoot();
	EXPECT_THROW(mpc.load(minimalGeoPilot(), "MPC"), ParameterException);
	PilotParams estimator;
	estimator.directory_ = repositoryRoot();
	EXPECT_THROW(estimator.load(minimalGeoPilot(false, false)), ParameterException);
}
