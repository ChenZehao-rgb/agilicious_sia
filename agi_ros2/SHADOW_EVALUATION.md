...
输出端收到伪造允许命令也会在映射/发送之前返回。正常飞行入口默认仍为原 RTK 路径。

## 启动

先构建并 source，准备真实机体参数目录；`pilot_ros2.yaml` 仅作为结构模板。
默认安装的 Iris 质量/惯量仍可用于软件调试，但不能用于解释真实机体 MPC 结果。
...
ros2 launch agi_ros2 shadow.launch.py \
  ...
  altitude_source:=msl \
  heading_confirmed:=true heading_correction_rad:=0.0 \
...
默认录制所有 topic（含隐藏 topic），输出至 `~/agi_bags/shadow_*`；使用 `bag_output`
...
缺少有效速度时不发布原子观测，不能用零速度补齐。TIMESYNC 只代表时钟对齐，
...
融合节点 `navigation_source=gnss` 仅订阅新观测；默认 `rtk` 路径不变。
...
MSP RC 和 STATUS 每秒查询 25 次，电池 2 次，配置回读每秒一次。原始事件照常记录。
只读配置包括 API/FC 标识与版本、BOXIDS、模式范围与 EXTRA、RX map/config、rates/deadband。
Override mask 和 failsafe 通过原生 MSP v2 `0x3010` 查询；发送 API 只允许这两个名字，
...
- AUX1 ARM、AUX2 MSP OVERRIDE、AUX3 FAILSAFE；默认有效范围 `[1700,2100)`。
  启动参数 `aux_low/aux_high` 可调整共同范围，必须与回读一致。
- 各授权模式只允许一个直接 OR 范围，不接受链接模式或 AND 逻辑。
- `msp_override_channels_mask=15`、`msp_override_failsafe=OFF`、midrc=1500。
- ACTUAL rates、center/max/expo 和 deadband 与 `bridge_config` 一致。

`MSP_RC` 的 AETR 可能来自 Override，不能当实体原始遥控；这里只使用已验证不被覆盖的 AUX。
STATUS 通过 BOXIDS 动态位置解码 ARM、MSP OVERRIDE、FAILSAFE，并读取 RX/FAILSAFE 禁止标志。
...
- IMU 完成标定、伴随 EKF 收敛尚无充分证据，始终 false 并说明原因；不使用
  sensor-present 位、未在校准位或 initialized 代替。这些未知量不阻止影子 MPC。

## 观察和验证

`/status` 仍表示原严格安全状态机，普通 GPS 和缺失证据会使它拒绝接管，这是预期结果。
`/computation_status` 独立报告 state_valid、mpc_success、warm_cycles、AUTO 参考活动状态、
reference_elapsed 和 solve_seconds。`trajectory_active` 表示 AUTO 参考已激活，空 CSV 时为悬停。
...
本次使用本机 ROS 2 Humble、合成数据与伪串口完成：
