...
ControlNode::ControlNode()
	...
	_state_sub = create_subscription<msg::FusedState>(
	                     "fused_state", 1,
	...
	_output_sub = create_subscription<msg::OutputStatus>(
	                      "output_status", 1,
	...
	_diagnostic_pub = create_publisher<std_msgs::msg::Float64MultiArray>(
...
void ControlNode::onState(msg::FusedState::ConstSharedPtr message) {
	...
	if (_state.reset_counter != 0 &&
	                message->reset_counter != _state.reset_counter) {
		_output_fault = true;
	}
	_state = *message;
}
...
void ControlNode::onOutputStatus(msg::OutputStatus::ConstSharedPtr message) {
	...
	if (!_output.clock_id.empty() &&
	                (_output.session_start != message->session_start ||
	                 _output.fault_count != message->fault_count)) {
		_output_fault = true;
	}
	_output = *message;
...
void ControlNode::tick() {
	_control_time =
	        alignedRosTime(*this, std::max({stampSeconds(_state.header.stamp),
	                                        stampSeconds(_authority.header.stamp),
	                                        stampSeconds(_health.header.stamp)}));
	const double wall = monotonicSeconds();
	if (_output_fault || (std::isfinite(_previous_clock) &&
	                      (_control_time < _previous_clock ||
	                       _control_time - _previous_clock > 0.25))) {
	...
	if (_state.initialized && _state.clock_id == _clock_id &&
	                _state.header.frame_id == "odom" &&
	                SafetyGate::fresh(wall, _state.published_steady_time, 0.010)) {
		state.t = stampSeconds(_state.header.stamp);
		state.p =
		        agi::Vector<3> {_state.position.x, _state.position.y, _state.position.z};
		state.v =
		        agi::Vector<3> {_state.velocity.x, _state.velocity.y, _state.velocity.z};
		state.q(agi::Quaternion(_state.orientation.w, _state.orientation.x,
		                        _state.orientation.y, _state.orientation.z));
		state.w = agi::Vector<3> {_state.body_rates.x, _state.body_rates.y,
		                          _state.body_rates.z
		                         };
		state.a = agi::Vector<3> {_state.acceleration.x, _state.acceleration.y,
		                          _state.acceleration.z
		                         };
	}
	agi::hardware::Evidence evidence;
	evidence.now = wall;
	evidence.imu_time =
	        _state.clock_id == _clock_id ? _state.imu_receive_time : kUnknownTime;
	evidence.rtk_time =
	        _state.initialized
	        ? wall - (_control_time - stampSeconds(_state.rtk_stamp))
	        : kUnknownTime;
	evidence.rc_time =
	        wall - (_control_time - stampSeconds(_authority.header.stamp));
	evidence.rc_link = _authority.rc_link &&
	                   SafetyGate::fresh(wall, _authority_receive_time, 0.1);
	evidence.armed = _authority.armed;
	evidence.auto_switch = _authority.auto_switch;
	evidence.kill = _authority.kill;
	evidence.rtk_fixed =
	        _state.rtk_fixed && SafetyGate::fresh(wall, _state.rtk_receive_time, 0.3);
	...
	const bool health_fresh =
	        SafetyGate::fresh(wall, _health_receive_time, 0.2) &&
	        SafetyGate::fresh(_control_time, stampSeconds(_health.header.stamp), 0.2);
	const bool output_fresh =
	        SafetyGate::fresh(wall, _output_receive_time, 0.05) &&
	        SafetyGate::fresh(wall, _output.steady_time, 0.05);
	...
	evidence.thrust_calibrated = health_fresh && _health.thrust_calibrated &&
	                             output_fresh && _output.thrust_calibrated;
	evidence.geofence_ok = health_fresh && _health.geofence_ok;
	evidence.msp_healthy = health_fresh && _health.transport_healthy &&
	                       output_fresh && _output.transport_healthy;
